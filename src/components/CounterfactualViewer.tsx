...
interface CounterfactualViewerProps {
  ...
  userId: string;
  sessionId: string;
  onCounterfactualSelected?: (recordingId: string, counterfactual: string) => void;
}
...
export const CounterfactualViewer: React.FC<CounterfactualViewerProps> = ({
  ...
  const generateCounterfactualsForStep = async (stepNumber: number) => {
    ...
    try {
      const result = await CounterfactualService.generateSingleCounterfactual(
        userId,
        recording
      );

      if (result.success) {
        const stepCounterfactuals = CounterfactualService.getCounterfactualsForStep(
  ...
  const selectCounterfactual = (stepNumber: number, counterfactualText: string) => {
    setStepData(prev => prev.map(step => 
      step.stepNumber === stepNumber 
      ...
      {!serviceAvailable && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
          <p className="text-red-700 text-sm">
            Counterfactual service is not available. Please ensure the FastAPI server is running on localhost:8000
          </p>
        </div>
      )}

      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">What If Alternatives</h2>
        <button
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-3 mb-4">
              ...
              <p className="text-gray-900">{stepData[activeStep].originalText}</p>
            </div>
          </div>
          ...
          {stepData[activeStep].isLoading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500 mb-4">No alternatives generated yet</p>
              <button
                onClick={() => generateCounterfactualsForStep(activeStep)}
                disabled={!serviceAvailable || !stepData[activeStep].originalText}
            ...
            <div className="space-y-3">
              <h4 className="font-medium text-gray-900">Alternative Responses:</h4>
              {stepData[activeStep].counterfactuals.map((cf) => (
                <div
                  key={cf.id}
                  className={`border rounded-lg p-3 cursor-pointer transition-all ${
                    stepData[activeStep].selectedCounterfactual === cf.counterfactualText
                      ? 'border-blue-500 bg-blue-50'
                      : 'border-gray-200 hover:border-blue-300'
                  }`}
                  onClick={() => selectCounterfactual(activeStep, cf.counterfactualText)}
                >
          ...
          {stepData[activeStep].selectedCounterfactual && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
              <p className="text-sm font-medium text-blue-900 mb-1">Your Selected Alternative:</p>
              <p className="text-blue-800">{stepData[activeStep].selectedCounterfactual}</p>
            </div>
          )}
        </div>
      )}

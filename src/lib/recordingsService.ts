...
export class RecordingsService {
   ...
   */
  static async getUserRecordingSessions(
    userId: string
    ...
    try {
      ...
      if (snapshot.empty) {
        ...
        for (const sess of sessionsSnap.docs) {
          const recordingsCol = collection(
            doc(db, "users", userId, "sessions", sess.id),
            "recordings"
          );
          const recSnap = await getDocs(recordingsCol);
          recSnap.docs.forEach((d) =>
            allRecordingDocs.push({ id: d.id, data: d.data() as Loose })
          );
        }
        ...
        const toTimestampDate = (v: unknown): Date =>
          v instanceof Timestamp ? v.toDate() : new Date(toStringSafe(v, ""));

        const manualRecordings: Recording[] = allRecordingDocs.map(
          ({ id, data }) => {
            ...
            const selectedAlt = counterfactuals?.selectedAlternative as
              | Loose
              | undefined;
            ...
            return {
              ...
              createdAt:
                data.createdAt instanceof Timestamp
                  ...
                  : new Date(toStringSafe(data.createdAt, "")),
              audioUri: toStringSafe(
                data.audioUri,
                undefined as unknown as string
              ),
              fileUrl: toStringSafe(
                data.fileUrl,
                undefined as unknown as string
              ),
              transcription: transcription
                ...
                : undefined,
              transcriptionText: toStringSafe(
                data.transcriptionText,
                undefined as unknown as string
              ),
              transcriptionStatus: toStringSafe(
                data.transcriptionStatus,
                "pending"
              ) as "pending" | "processing" | "completed" | "failed",
              activitySummary: activitySummary
                ? {
                    primaryActivity: toStringSafe(
                      activitySummary.primaryActivity,
                      "unknown"
                    ),
              ...
              counterfactuals: counterfactuals
                ? {
                    ...
                    questionIndex: toNumberSafe(
                      counterfactuals.questionIndex,
                      0
                    ),
                  }
                : undefined,
        ...
        if (manualRecordings.length > 0) {
          const sessions =
            this.groupRecordingsBySessionNumber(manualRecordings);
          return sessions;
        }
      }
        ...
        return {
          ...
          transcription: data.transcription
            ? {
                ...
                transcribedAt:
                  data.transcription.transcribedAt instanceof Timestamp
          ...
          activitySummary: data.activitySummary
            ? {
                primaryActivity:
                  data.activitySummary.primaryActivity || "unknown",
          ...
          counterfactuals: data.counterfactuals
            ? {
                ...
                selectedAlternative: data.counterfactuals.selectedAlternative
                  ...
                  : undefined,
                generatedAt:
                  data.counterfactuals.generatedAt instanceof Timestamp
   ...
   */
  private static groupRecordingsBySessionNumber(
    recordings: Recording[]
  ): RecordingSession[] {
    // Separate those with sessionNumber from legacy ones
    const withSession = recordings.filter(
      (r) => typeof r.sessionNumber === "number"
    ) as Required<Recording>[];
    const legacy = recordings.filter(
      (r) => typeof r.sessionNumber !== "number"
    );
    ...
    const sessionsFromNumber: RecordingSession[] = Array.from(
      groups.entries()
    ).map(([n, recs]) => {
      ...
      const completedAt = new Date(
        Math.max(...sorted.map((r) => r.createdAt.getTime()))
      );
  ...
  private static groupLegacyRecordings(
    recordings: Recording[]
  ): RecordingSession[] {
    ...
    for (const r of sorted) {
      ...
      if (bucket.length === 5) {
        const completedAt = new Date(
          Math.max(...bucket.map((x) => x.createdAt.getTime()))
        );
        sessions.push({
  ...
  private static createSession(recordings: Recording[]): RecordingSession {
    const sortedByStep = [...recordings].sort(
      (a, b) => (a.stepNumber || 0) - (b.stepNumber || 0)
    );
    const completedAt = new Date(
      Math.max(...recordings.map((r) => r.createdAt.getTime()))
    );
    return {
  ...
  static async associateSessionsWithWeeklyPlans(userId: string): Promise<void> {
    try {
      console.log(
        "WeeklyPlanService integration temporarily disabled for user:",
        userId
      );
   ...
   */
  static async getUserRecordingsByDay(
    userId: string
  ): Promise<Record<string, RecordingSession[]>> {
    const sessions = await this.getUserRecordingSessions(userId);
  ...
  static calculateStatsFromSessions(sessions: RecordingSession[]): {
    ...
    const transcribedRecordings = sessions.reduce(
      (sum, session) =>
        sum +
        session.recordings.filter((r) => r.transcriptionStatus === "completed")
          .length,
      0
    );
